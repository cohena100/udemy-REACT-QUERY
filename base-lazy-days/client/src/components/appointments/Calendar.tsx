  ...
  return (
    <Box>
      <HStack mt={10} spacing={8} justify="center">
        <IconButton aria-label="previous month" onClick={() => updateMonthYear(-1)} icon={<TiArrowLeftThick />} isDisabled={monthYear.startDate < currentDate} />
        ...
        <Checkbox variant="flushed" width="48" position="absolute" right="10px" checked={!showAll} defaultChecked onChange={() => setShowAll((prevValue) => !prevValue)}>
          Only show available
        </Checkbox>
      </HStack>
      <Grid templateColumns="repeat(7, 1fr)" gap={4} my={5} mx={10}>
        ...
        {[...Array(monthYear.lastDate)].map((_, i) => (i > 0 ? <DateBox key={i} date={i + 1} appointments={appointments[i + 1]} /> : null))}
      </Grid>
      <UserAppointments />
    </Box>
  );
